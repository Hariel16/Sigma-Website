        ...
        if (isset($_FILES[$field]) && $_FILES[$field]['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($file['tmp_name'], $destination)) {
                ...
                if ($old_file = $result->fetch_assoc()) {
                    if ($old_file['setting_value'] && file_exists($old_file['setting_value'])) {
                        unlink($old_file['setting_value']);
                    }
                }
                $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['publish_results'])) {
    ...
    if ($stmt->execute()) {
        $_SESSION['success'] = "Résultats publiés avec succès.";
    } else {
...
$stmt = $conn->prepare("SELECT c.*, e.title as election_title, u.full_name 
                        FROM candidates c 
...
foreach ($elections as $election) {
    if (date('Y-m-d H:i:s') > $election['end_date']) {
        $stmt = $conn->prepare("SELECT c.position, c.id as candidate_id, u.full_name, 
                                COUNT(v.id) as vote_count 
                                FROM candidates c 
                                LEFT JOIN votes v ON v.candidate_id = c.id AND v.election_id = ? 
                                LEFT JOIN users u ON c.user_id = u.id 
                                WHERE c.election_id = ? 
                                GROUP BY c.id, c.position, u.full_name");
        $stmt->bind_param("ii", $election['id'], $election['id']);
        $stmt->execute();
        $result = $stmt->get_result();
        $election_results = [];
        while ($row = $result->fetch_assoc()) {
            if (!isset($election_results[$row['position']])) {
                $election_results[$row['position']] = [];
            }
            $election_results[$row['position']][] = [
                'name' => $row['full_name'],
        ...
        $stmt = $conn->prepare("SELECT position, COUNT(*) as blank_votes 
                                FROM votes 
        ...
        while ($row = $result->fetch_assoc()) {
            $election_results[$row['position']][] = [
                'name' => 'Votes blancs',
                'votes' => $row['blank_votes'],
                'percentage' => 0
            ];
        }
        $stmt->close();

        foreach ($election_results as $position => &$position_results) {
            $total_votes = array_sum(array_column($position_results, 'votes'));
            if ($total_votes > 0) {
        ...
        $results[$election['id']] = $election_results;
    }
}
...
<head>
    ...
    <style>
        ...
        .sidebar {
            width: 280px;
            background: linear-gradient(180deg, var(--sigma-blue) 0%, var(--sigma-blue-light) 100%);
            color: white;
            position: fixed;
            height: 100vh;
            overflow-y: auto;
...
<body>
    <div class="admin-container">
        ...
        <div class="sidebar" id="sidebar">
            ...
            <div class="sidebar-nav">
                ...
                <div class="nav-item" data-target="bureau">
                ...
                <div class="nav-item" data-target="suggestions">
                    <i class="fas fa-lightbulb"></i>
                    <span>Suggestions</span>
                </div>
                <div class="nav-item" data-target="events-management">
                ...
                <div class="nav-item" data-target="general-config">
                    <i class="fas fa-cog"></i>
                    <span>Configuration</span>
                </div>
                <div class="nav-item" data-target="regulations">
                    <i class="fas fa-book"></i>
                    <span>Règlement</span>
                </div>
                <div class="nav-item" data-target="objectifs">
                    <i class="fas fa-bullseye"></i>
                    <span>Objectifs</span>
                </div>
                <div class="nav-item" data-target="elections">
                    <i class="fas fa-vote-yea"></i>
                    <span>Élections</span>
                </div>
                <div class="nav-item" data-target="candidates">
        ...
        <div class="main-content">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="contact_address" class="form-input" rows="2" required><?php echo htmlspecialchars($general_config['contact_address']); ?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="p-4 border rounded-lg">
                                <label class="block text-sm font-medium mb-2">Logo de l'administration</label>
                                <div class="flex items-center space-x-4 mb-2">
            ...
            <section id="results" class="section">
                <div class="card">
                    <h2 class="text-2xl font-bold mb-6"><i class="fas fa-chart-bar text-blue-600 mr-2"></i>Résultats des élections et Suivi des votes</h2>
                    
                    <?php foreach ($elections as $election): ?>
                        <div class="mb-8 border rounded-lg p-6">
                            <div class="flex justify-between items-start mb-4">
                                <h3 class="text-xl font-semibold"><?php echo htmlspecialchars($election['title']); ?></h3>
                                <div class="text-right">
                                    ...
                                    <?php if (isset($election_stats[$election['id']])): ?>
                                        <div class="text-sm text-gray-600 mt-1">
                                            Participation: <?php echo $election_stats[$election['id']]['participation_rate']; ?>%
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
                            ...
                            ?>
                                <div class="bg-gray-50 p-4 rounded-lg mb-4">
                                    <h4 class="font-semibold mb-3">Suivi de la participation en temps réel</h4>
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                                        ...
                                        <div>
                                            <div class="text-2xl font-bold text-green-600"><?php echo $stats['voters']; ?></div>
                                            <div class="text-sm text-gray-600">Ont voté</div>
                                        </div>
                                        <div>
                                            <div class="text-2xl font-bold text-orange-600"><?php echo $stats['remaining_voters']; ?></div>
                                            <div class="text-sm text-gray-600">Restants</div>
                                        </div>
                                    </div>
                                    <div class="mt-3">
                                        <div class="flex justify-between text-sm mb-1">
                                            <span>Taux de participation</span>
                                            <span><?php echo $stats['participation_rate']; ?>%</span>
                                        </div>
                                        <div class="w-full bg-gray-200 rounded-full h-2">
                                            <div class="bg-green-600 h-2 rounded-full" style="width: <?php echo $stats['participation_rate']; ?>%"></div>
                                        </div>
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <?php if (date('Y-m-d H:i:s') > $election['end_date']): ?>
                                <h4 class="text-lg font-semibold mb-3">Résultats détaillés par poste</h4>
                                <?php if (isset($results[$election['id']])): ?>
                                    <?php foreach ($results[$election['id']] as $position => $position_results): ?>
                                        <div class="bg-white border p-4 rounded-lg mb-4">
                                            <h5 class="font-medium mb-3 text-blue-700"><?php echo htmlspecialchars($position); ?></h5>
                                            <?php
                                                $max_votes = max(array_column($position_results, 'votes'));
                                                $winner = array_reduce($position_results, function($carry, $item) use ($max_votes) {
                                                    return ($item['votes'] === $max_votes && $item['name'] !== 'Votes blancs') ? $item : $carry;
                                                }, null);
                                            ?>
                                            <?php if ($winner): ?>
                                                <div class="flex items-center justify-between mb-3 p-3 bg-green-50 rounded-lg">
                                                    <div class="flex items-center space-x-3">
                                                        ...
                                                        <div>
                                                            <div class="font-semibold"><?php echo htmlspecialchars($winner['name']); ?></div>
                                                            <div class="text-sm text-gray-600"><?php echo htmlspecialchars($winner['percentage']); ?>% (<?php echo $winner['votes']; ?> votes)</div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="flex justify-between items-center p-2 hover:bg-gray-50 rounded">
                                                        <div class="flex items-center space-x-3">
                                                            <?php if ($result['name'] !== 'Votes blancs'): ?>
                                                                <span class="font-medium"><?php echo htmlspecialchars($result['name']); ?></span>
                                                            <?php else: ?>
                                                                <span class="text-gray-500"><i class="fas fa-ban mr-1"></i><?php echo htmlspecialchars($result['name']); ?></span>
                                                            <?php endif; ?>
                                                        </div>
                                                        <div class="text-right">
                                                            <span class="font-medium"><?php echo htmlspecialchars($result['percentage']); ?>%</span>
                                                            <span class="text-sm text-gray-500 ml-2">(<?php echo $result['votes']; ?> votes)</span>
                                                        </div>
                                                    </div>
                                                <?php endforeach; ?>
                                            </div>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <p class="text-gray-500">Aucun résultat disponible pour cette élection.</p>
                                <?php endif; ?>
                                ...
                                <div class="flex space-x-3 mt-4">
                                    <?php if ($election['results_published']): ?>
                                        <form method="POST" class="inline">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                                            <input type="hidden" name="election_id" value="<?php echo $election['id']; ?>">
                                            <button type="submit" name="unpublish_results" class="btn btn-warning">
                                        ...
                                        <span class="bg-green-100 text-green-800 px-3 py-2 rounded text-sm font-medium">
                                            <i class="fas fa-check mr-1"></i>Résultats publiés
                                        </span>
                                    <?php else: ?>
                                        <form method="POST" class="inline">
                                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                                        ...
                                        <span class="bg-red-100 text-red-800 px-3 py-2 rounded text-sm font-medium">
                                            <i class="fas fa-times mr-1"></i>Résultats masqués
                                        </span>
                                    <?php endif; ?>
                                </div>
                            <?php else: ?>
                    ...
                    <?php if (empty($elections)): ?>
                        <p class="text-gray-500">Aucune élection créée pour le moment.</p>
                    <?php endif; ?>
                </div>
            </section>
        </div>
    ...
    <script>
        ...
        tinymce.init({
            selector: 'textarea[name="content"], textarea#edit_content',
            height: 300,
