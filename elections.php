...
function getProfilePicture($path) {
    ...
    return 'assets/images/profile_pic.jpeg';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_vote'])) {
    ...
    try {
        ...
        foreach ($valid_positions as $position) {
            ...
            $candidate_id = ($value === 'blank') ? NULL : (int)$value;
            $is_blank = ($value === 'blank') ? 1 : 0;

            // Vérifier si l'utilisateur a déjà voté pour CETTE position spécifique
            if (isset($voted_positions[$position])) {
                throw new Exception("Vous avez déjà voté pour la position : $position");
            }
            ...
            $stmt = $conn->prepare("INSERT INTO votes (election_id, user_id, candidate_id, position, is_blank, voted_at)
                                VALUES (?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("iisii", $election['id'], $_SESSION['user_id'], $candidate_id, $position, $is_blank);
            $stmt->execute();
            $stmt->close();
        ...
        if ($votes_submitted !== count($valid_positions)) {
            throw new Exception("Vous devez voter pour toutes les positions.");
        }
...
if ($election && $show_results) {
    foreach ($candidates_by_position as $position => $candidates) {
        ...
        foreach ($candidates as $candidate) {
            // Utiliser directement le vote_count déjà calculé dans la requête principale
        ...
        // Récupérer les votes blancs
        $stmt = $conn->prepare("SELECT COUNT(*) as blank_votes 
                              FROM votes 
                              WHERE election_id = ? AND position = ? AND is_blank = 1");
        $stmt->bind_param("is", $election['id'], $position);
        $stmt->execute();
        $blank_votes = $stmt->get_result()->fetch_assoc()['blank_votes'];
        $stmt->close();
        ...
        if ($total_votes > 0) {
            foreach ($results[$position] as &$result) {
                $result['percentage'] = round(($result['votes'] / $total_votes) * 100, 1);
            }
        }
        
        // Trier par nombre de votes décroissant
        usort($results[$position], function($a, $b) {
...
<body class="bg-gray-50">
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="w-full bg-white bg-opacity-30 rounded-full h-2.5">
                                <div class="bg-yellow-400 h-2.5 rounded-full" style="width: <?php echo $progress; ?>%"></div>
                            </div>
    ...
    <div id="videoModal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg p-4 max-w-4xl w-full mx-4">
            ...
            <video id="candidateVideo" controls class="w-full h-auto max-h-[70vh]">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                ...
                new Chart(ctx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            title: {
                                display: true,
                                text: 'Répartition des votes par position',
                                font: {
                                    size: 16
                                }
                            }
            ...
            document.getElementById('candidateVideo').addEventListener('error', function() {
                alert('Impossible de charger la vidéo. Veuillez vérifier le lien.');
                closeVideoModal();
            });
        });
    </script>
